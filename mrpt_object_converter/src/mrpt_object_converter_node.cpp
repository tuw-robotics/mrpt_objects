...
ObjectConverterNode::ParametersNode::ParametersNode() : nh( "~" )
{
  ...
  nh.param<std::string>( "target_frame", target_frame_name, std::string( "r0/laser0" ));
...
void ObjectConverterNode::init()
{
  ...
  sub_object_detections_ = n_.subscribe( param()->subscriber_topic_name, 1,
                                         &ObjectConverterNode::callbackObjectDetections, this );
...
bool ObjectConverterNode::convert( const tuw_object_msgs::ObjectWithCovariance &_obj,
                                   mrpt::obs::CObservationObject::TMeasurement &_meas )
{
  ...
  if ( !getTF( param()->target_frame_name, map_pose_ ))
  {
    ROS_WARN( "could not get laser tf" );
...
bool ObjectConverterNode::getTF( std::string target_frame, mrpt::poses::CPose3D &des )
{
  std::string world_frame_id = tf::resolve( param()->tf_prefix, param()->world_frame_name );
  std::string target_frame_id = target_frame;
  std::string key = world_frame_id + "->" + target_frame_id;
  tf::StampedTransform transform;
  
  auto elem = static_tf_.find( key );
  if ( elem != static_tf_.end())
  {
    des = elem->second;
    return true;
  }
  ...
  try
  {
    if ( param()->debug )
      ROS_INFO(
          "debug: updateLaserPose(): world_frame_id='%s' target_frame_id='%s'",
          world_frame_id.c_str(), target_frame_id.c_str());
    
    listenerTF_.lookupTransform( world_frame_id, target_frame_id, ros::Time( 0 ), transform );
    ...
    des.setRotationMatrix( Rdes );
    static_tf_.insert( std::make_pair( key, des ));
    //ROS_INFO("Static tf '%s' with '%s'",
    //         key.c_str(), pose.asString().c_str());
  }
  catch (tf::TransformException ex)
  {
    ROS_WARN( "MRPTObjectConverterNode::getTF( %s )\n", key.c_str());
    ROS_ERROR( "%s", ex.what());
